...
// --- LOGICA DE HORARIO DE CONTACTO (BLOQUEO DOBLE) ---
window.checkContactStatus = () => {
    const btnSpec = document.getElementById('btn-specialist-chat');
    const badgeSpec = document.getElementById('spec-badge');
    const discSpec = document.getElementById('spec-disclaimer');
    const btnAdmin = document.getElementById('btn-admin-chat');
    const badgeAdmin = document.getElementById('admin-badge');

    if (!btnSpec || !btnAdmin) return;
    ...
    if (isOpen) {
        // --- ABIERTO ---
        // Especialista
        btnSpec.classList.remove('opacity-50', 'pointer-events-none', 'grayscale', 'bg-slate-400');
        btnSpec.classList.add('bg-emerald-500', 'hover:bg-emerald-600', 'shadow-lg');
        btnSpec.textContent = "Iniciar Chat Clínico";
        if(badgeSpec) badgeSpec.innerHTML = '<span class="px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-600 text-[9px] font-black uppercase tracking-widest border border-emerald-200">● En línea</span>';
        if(discSpec) discSpec.classList.remove('hidden'); // Mostrar disclaimer

        // Admin
        btnAdmin.classList.remove('opacity-50', 'pointer-events-none', 'grayscale');
        btnAdmin.classList.add('shadow-lg', 'hover:scale-[1.02]');
        btnAdmin.textContent = "Chat Administrativo";
        if(badgeAdmin) badgeAdmin.innerHTML = '<span class="px-2 py-0.5 rounded-full bg-blue-100 text-[#2E4982] text-[9px] font-black uppercase tracking-widest border border-blue-200">● En línea</span>';

    } else {
        // --- CERRADO ---
        // Especialista
        btnSpec.classList.add('opacity-50', 'pointer-events-none', 'grayscale', 'bg-slate-400');
        btnSpec.classList.remove('bg-emerald-500', 'hover:bg-emerald-600', 'shadow-lg');
        btnSpec.textContent = "Cerrado (Lun-Vie 10am-6pm)";
        if(badgeSpec) badgeSpec.innerHTML = '<span class="px-2 py-0.5 rounded-full bg-slate-100 text-slate-400 text-[9px] font-black uppercase tracking-widest border border-slate-200">● Cerrado</span>';
        if(discSpec) discSpec.classList.add('hidden'); // Ocultar disclaimer

        // Admin
        btnAdmin.classList.add('opacity-50', 'pointer-events-none', 'grayscale');
        btnAdmin.classList.remove('shadow-lg', 'hover:scale-[1.02]');
        btnAdmin.textContent = "Cerrado (Lun-Vie 10am-6pm)";
        if(badgeAdmin) badgeAdmin.innerHTML = '<span class="px-2 py-0.5 rounded-full bg-slate-100 text-slate-400 text-[9px] font-black uppercase tracking-widest border border-slate-200">● Cerrado</span>';
    }
};
